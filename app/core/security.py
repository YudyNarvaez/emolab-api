...
def create_jwt_token(subject: int, exp_secs: int, refresh: bool):
    ...
    to_encode: dict[str, Union[int, str, bool]] = {
        "issued_at": issued_at,
...
def generate_access_token_response(subject: int):
...
def generate_otp_token(subject: int, expires_secs: int):
    access_token, expires_at, issued_at = create_jwt_token(
        subject, expires_secs, refresh=False
    )
